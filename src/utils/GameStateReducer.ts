...
const processFoodConsumption = (gameState: GameState) => {
    ...
    return {...gameState, items: {...gameState.items, berries: {amount: currentFood - foodToConsume, multiplier: gameState?.items?.berries?.multiplier ?? 1}}};
}
...
const gameStateReducer = (state: GameState, action: Action) => {
    switch (action.type) {
        ...
        case 'gameTick':
            // Do things
            state = processFoodConsumption(state)
            return state;
        case 'timeTick':
